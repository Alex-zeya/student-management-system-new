...
const StudentDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const [myCourses, myTasks, latestAnnouncements] = await Promise.all([
          courseAPI.myCourses(),
          taskAPI.myTasks(),
        ...
        setAnnouncements(latestAnnouncements.data);
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    if (activeTab === 'home') {
      const refreshHome = async () => {
        setTabLoading(true);
        try {
          const [myTasks, latestAnnouncements] = await Promise.all([
            taskAPI.myTasks(),
            announcementAPI.getAnnouncements(),
          ]);
          setTasks(myTasks.data);

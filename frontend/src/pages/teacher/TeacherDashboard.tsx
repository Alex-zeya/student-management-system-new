...
const TeacherDashboard: React.FC = () => {
  ...
  const fmtDate = (s: string) => new Date(s).toLocaleString('en-GB', { dateStyle: 'short', timeStyle: 'short' });
            ...
            <div className="space-y-4">
              {announcements.map(ann => (
                <div key={ann.id} className={`glass-effect p-5 ${ann.is_pinned ? 'border-l-4 border-green-500' : ''}`}>
                  ...
                  <p className="text-sm text-gray-600 mt-3 whitespace-pre-wrap">{ann.content}</p><p className="text-xs text-gray-400 mt-3">{new Date(ann.created_at).toLocaleDateString('en-GB')}</p>
                </div>
              ))}
            </div>
          )}

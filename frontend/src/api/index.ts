...
export const resourceAPI = {
  getResources: (courseId?: number) =>
    courseId ? api.get<CourseResource[]>(`/resources/course_resources/?course_id=${courseId}`) : api.get<CourseResource[]>('/resources/'),
  getResource: (id: number) => api.get<CourseResource>(`/resources/${id}/`),
  createResource: (data: FormData) => api.post<CourseResource>('/resources/', data, {
    headers: { 'Content-Type': 'multipart/form-data' },
  }),
  updateResource: (id: number, data: FormData) => api.put<CourseResource>(`/resources/${id}/`, data, {
    headers: { 'Content-Type': 'multipart/form-data' },
  }),
...
// Enrollment related APIs (for administrator maintenance)
